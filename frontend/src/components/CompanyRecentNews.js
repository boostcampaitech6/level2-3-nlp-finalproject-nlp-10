  ...
  return (
    <>
      ...
      {newsSamsung.map((news, idx) => (
        <Box key={idx} sx={{ p: 2, mb: 2, border: "1px solid lightgray", borderRadius: "10px" }}>
          ...
          <Box sx={{ pl: 4 }}>
            <List sx={{ listStyleType: 'square' }}>
              {news['summary'].map((summary, idx) => (
                <ListItem key={idx} sx={{ p: 0.5, display: 'list-item', fontSize: "1rem", fontFamily: "omyu_pretty" }}>
                  {summary}
                </ListItem>
              ))}
            </List>
          </Box>
          <Box sx={{ pl: 3, font: "0.8rem light gray", fontFamily: "omyu_pretty" }}>
            {news['sentiment']} | 관련 뉴스 {news['relatedNews']}건 | {news['keywords'].map((keyword, idx) => (<span key={idx} >#{keyword} </span>))}
          </Box>
        </Box>))}
    </>
  )
